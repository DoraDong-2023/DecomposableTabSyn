...
def evaluate_single_table_metrics(real_data, synthetic_data, metadata, save_path="results"):
    ...
    summary = {
        ...
        "Properties": property_score,
        "Metrics": metrics,
        "Metrics Overall Score": np.mean([v for v in metrics.values() if v is not None and not np.isnan(v)]),
        "Details": details,
    }
    print("---" * 20)
...
def train_and_evaluate_synthesizer(synthesizer, real_data, metadata, num_samples, save_path):
    ...
    return metrics, fit_time, sample_time, decomposition_time, synthetic_data
...
def benchmark_single_synthesizer(datasets, new_synthesizer_fn, num_samples=1000, row_sizes=[1000], save_path="results", seed=42):
    ...
    for dataset_name in tqdm(datasets, desc="Evaluating"):
        ...
        for num_rows in row_sizes:
            ...
            if (experiment_save_path / "results.json").exists():
                print(f"Results on {dataset_name} with {num_rows} rows already exist. Skipping ...")
                with open(experiment_save_path / "results.json", "r") as f:
                    eval_results = json.load(f)
                print(f"Results on {dataset_name} loaded from {dataset_save_path}")
                # print metrics
                print("---" * 20)
                print("Quality Report:")
                print(f" - Overall Score: {eval_results['Overall Score']}")
                print(" - Properties:")
                for k, v in eval_results['Properties'].items():
                    print(f"   - {k}: {v}")
                print(" - Metrics:")
            ...
            # Train and evaluate
            metrics, fit_time, sample_time, decomposition_time, synthetic_data = train_and_evaluate_synthesizer(synthesizer, data, metadata, num_samples, experiment_save_path)
            ...
            eval_results.update(metrics)  # Add metrics to the row
            
            with open(experiment_save_path / "results.json", "w") as f:
                json.dump(eval_results, f, indent=4)
            synthetic_data.to_csv(experiment_save_path / "synthetic_data.csv", index=False)
            print(f"Results on {dataset_name} saved to {experiment_save_path}")
...
def main(
    ...
    for dataset_name in dataset_names:
        assert dataset_name in available_dataset_names, f"Invalid dataset name {dataset_name}. Available dataset names are: {available_dataset_names}"
